...
int main(int argc, char *argv[]) {
   ...
   * -w, --wait: Time to wait in seconds for a host before falling back to
   * localhost (default: 0)
   */
  ...
  for (; nextarg < argc; nextarg++) {
    if (strequ("-s", argv[nextarg]) || strequ("--server", argv[nextarg])) {
      ...
      debugMode = true;
    } else if (strequ("-w", argv[nextarg]) || strequ("--wait", argv[nextarg])) {
      if (++nextarg >= argc) {
        usage(argv[0]);
        return -1;
      }
  ...
  if (!client_sock) {
    ...
    do {
      ...
      gettimeofday(&end, 0);
    } while (resolved_name.empty() &&
             (timeout == -1 || ((end.tv_sec - begin.tv_sec) > timeout)));
  }
...
void usage(const char *prog) {
  fprintf(stderr,
          ...
          "[-D|--debug] [-t|--type <typestr>] [-w|--wait <timeout>] <command> "
          "[args] \n\n",
          prog);
}
