...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    if (strequ("-p", argv[i]) || strequ("--port", argv[i])) {
      ...
      debugMode = true;
    } else if (strequ("-H", argv[i]) || strequ("--hosts-info-file", argv[i])) {
      if (++i >= argc) {
        usage(argv[0]);
        return -1;
      }
  ...
  while (1) {
    ...
    if (result > 0) {  // something is available to be read
      ...
      if (Smaskisset(server)) {
        ...
        if (sock_req == NULL) {
          DMUCS_DEBUG(
              (stderr, "ERROR: Saccept returns 0: %s\n", strerror(errno)));
        } else {
          addFd(sock_req);
...
static void spawn_silent_thread() {
  ...
  pthread_t thread_id;
  if (pthread_create(&thread_id, &tattr, doSilentSearch, (void *)NULL) != 0) {
...
static void *doSilentSearch(void *bogus /* not used */) {
  while (1) {
...
static void *updateStats(void *bogus /* not used */) {
...
static void handleReq(Socket *sock_req, DmucsDb *db) {
  char buf[BUFSIZE];
...
static char *peer2buf(const Socket *sock, char *buf) {
  struct sockaddr sck;
