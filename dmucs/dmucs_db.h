...
class DmucsDpropDb {
 private:
  ...
  typedef std::map<const Socket *, const unsigned int> dmucs_assigned_cpus_t;
 ...
 public:
  ...
  void assignCpuToClient(const unsigned int clientIp, const Socket *cpuIp);
  ...
  void addCpusToTier(int tierNum, const unsigned int ipAddr, const int numCpus);
...
class MutexMonitor {
 public:
  MutexMonitor(pthread_mutex_t *m) : m_(m) { pthread_mutex_lock(m_); }
  ~MutexMonitor() { pthread_mutex_unlock(m_); }
...
class DmucsDb {
 ...
 public:
  ...
  void assignCpuToClient(const unsigned int clientIp, const DmucsDprop dprop,
                         const Socket *sock);
  void moveCpus(DmucsHost *host, int oldTier, int newTier) {
    MutexMonitor m(&mutex_);
    // Assume the DmucsDpropDb is definitely there.
    return dbDb_.find(host->getDprop())
        ->second.moveCpus(host, oldTier, newTier);
  }
  int delCpusFromTier(DmucsHost *host, int tier, unsigned int ipAddr) {
    MutexMonitor m(&mutex_);
    // Assume the DmucsDpropDb is definitely there.
    return dbDb_.find(host->getDprop())->second.delCpusFromTier(tier, ipAddr);
  }
  ...
  void handleSilentHosts() {
    MutexMonitor m(&mutex_);
    for (dmucs_dprop_db_iter_t itr = dbDb_.begin(); itr != dbDb_.end(); ++itr) {
...
inline bool DmucsDpropDb::DmucsHostCompare::operator()(DmucsHost *lhs,
                                                       DmucsHost *rhs) const {
