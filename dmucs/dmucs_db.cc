...
const char *dprop2cstr(DmucsDprop d) { return d.c_str(); }
...
void DmucsDb::assignCpuToClient(const unsigned int clientIp,
                                const DmucsDprop dprop, const Socket *sock) {
...
void DmucsDb::releaseCpu(const Socket *sock) {
 ...
 * exception.
 */
DmucsHost *DmucsDpropDb::getHost(const struct in_addr &ipAddr) {
...
bool DmucsDpropDb::haveHost(const struct in_addr &ipAddr) {
  DmucsHost host(ipAddr, dprop_, 0, 0);
 ...
 */
unsigned int DmucsDpropDb::getBestAvailCpu() {
...
void DmucsDpropDb::assignCpuToClient(const unsigned int hostIp,
                                     const Socket *sock) {
  struct in_addr t2;
  t2.s_addr = hostIp;
...
void DmucsDpropDb::releaseCpu(const Socket *sock) {
...
std::string DmucsDpropDb::serialize() {
  /*
  ...
  for (dmucs_host_set_iter_t itr = allHosts_.begin(); itr != allHosts_.end();
       ++itr) {
    struct in_addr in;
...
void DmucsDpropDb::addNewHost(DmucsHost *host) {
  /*
...
void DmucsDpropDb::addToHostSet(dmucs_host_set_t *theSet, DmucsHost *host) {
...
void DmucsDpropDb::delFromHostSet(dmucs_host_set_t *theSet, DmucsHost *host) {
...
void DmucsDpropDb::addToAvailDb(DmucsHost *host) {
  addToHostSet(&availHosts_, host);
  addCpusToTier(host->getTier(), host->getIpAddrInt(), host->getNumCpus());
}

void DmucsDpropDb::delFromAvailDb(DmucsHost *host) {
...
void DmucsDpropDb::addToOverloadedDb(DmucsHost *host) {
  addToHostSet(&overloadedHosts_, host);
}

void DmucsDpropDb::delFromOverloadedDb(DmucsHost *host) {
  delFromHostSet(&overloadedHosts_, host);
}

void DmucsDpropDb::addToSilentDb(DmucsHost *host) {
  addToHostSet(&silentHosts_, host);
}

void DmucsDpropDb::delFromSilentDb(DmucsHost *host) {
  delFromHostSet(&silentHosts_, host);
}

void DmucsDpropDb::addToUnavailDb(DmucsHost *host) {
  addToHostSet(&unavailHosts_, host);
}

void DmucsDpropDb::delFromUnavailDb(DmucsHost *host) {
  delFromHostSet(&unavailHosts_, host);
}

/* Add "numCpus" copies of the ipaddress to the list in the given tier. */
void DmucsDpropDb::addCpusToTier(int tierNum, const unsigned int ipAddr,
                                 const int numCpus) {
  /*
...
void DmucsDpropDb::moveCpus(DmucsHost *host, int oldTier, int newTier) {
...
int DmucsDpropDb::delCpusFromTier(int tier, unsigned int ipAddr) {
...
void DmucsDpropDb::handleSilentHosts() {
  for (dmucs_host_set_iter_t itr = allHosts_.begin(); itr != allHosts_.end();
       ++itr) {
    if ((*itr)->seemsDown()) {
      (*itr)->silent();
 ...
 * collection period.
 */
void DmucsDpropDb::getStatsFromDb(int *served, int *max, int *totalCpus) {
