...
int main(int argc, char *argv[]) {
   ...
   * o Send a supplied or my IP address in a 'remove host' or 'add host'
   * request, depending on the name of the executable. o Close the client
   * socket.
   */
   ...
   * -ip <address>: The machine IP to add or remove from the server (default:
   * localhost's IP).
   */
  ...
  for (; nextarg < argc; nextarg++) {
    ...
    } else if (strequ("-t", argv[nextarg]) || strequ("--type", argv[nextarg])) {
      if (++nextarg >= argc) {
        usage(argv[0]);
        return -1;
      }
  ...
  DMUCS_DEBUG(
      (stderr, "Writing -->%s<-- to the server\n", clientReqStr.str().c_str()));
...
void usage(const char *prog) {
  fprintf(stderr,
          ...
          "[-t|--type <str>] [-D|--debug] [-ip <address>] [--host <hostname>] "
          "<command> [args] \n\n",
          prog);
}
