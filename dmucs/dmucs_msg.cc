...
DmucsMsg *DmucsMsg::parseMsg(Socket *sock, const char *buffer) {
  ...
  if (strncmp(buffer, "host", 4) == 0) {
    ...
    if (sscanf(buffer, "load %s %f %f %f %s", machname, &ldavg1, &ldavg5,
               &ldavg10, dpropstr) != 5) {
      if (sscanf(buffer, "load %s %f %f %f", machname, &ldavg1, &ldavg5,
                 &ldavg10) != 4) {
    ...
    DMUCS_DEBUG((stderr,
                 "host %s: ldAvg1 %2.2f, ldAvg5 %2.2f, "
                 "ldAvg10 %2.2f, dprop '%s'\n",
                 machname, ldavg1, ldavg5, ldavg10, dpropstr));
    return new DmucsLdAvgMsg(clientIp, host, ldavg1, ldavg5, ldavg10, dpropstr);
...
void DmucsHostReqMsg::handle(Socket *sock, const char *buf) {
  ...
  try {
    ...
    std::string resolved_name = DmucsHost::resolveIp2Name(cpuIpAddr, dprop_);
...
void DmucsLdAvgMsg::handle(Socket *sock, const char *buf) {
  DmucsDb *db = DmucsDb::getInstance();
  ...
  try {
    ...
    if (host->isSilent() || (host->isOverloaded() && host->getTier() != 0)) {
  ...
  if (host) {
    ...
    NSDictionary *Info = [NSDictionary
        dictionaryWithObjectsAndKeys:HostName, @"HostName", LoadAvg1,
                                     @"LoadAvg1", LoadAvg5, @"LoadAvg5",
                                     LoadAvg10, @"LoadAvg10", nil];
    [Notifier postNotificationName:@"dmucsLoadAvg"
                            object:@"DMUCS"
                          userInfo:Info
                           options:(NSUInteger)NSNotificationPostToAllSessions];
  }
#endif
  removeFd(sock);
}

void DmucsStatusMsg::handle(Socket *sock, const char *buf) {
  DmucsDb *db = DmucsDb::getInstance();
...
void DmucsMonitorReqMsg::handle(Socket *sock, const char *buf) {
