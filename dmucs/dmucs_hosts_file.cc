...
DmucsHostsFile::DmucsHostsFile(const std::string &hostsInfoFile)
    : lastFileChangeTime_(0), hostsInfoFile_(hostsInfoFile) {
  readFileIntoDb();
...
void DmucsHostsFile::readFileIntoDb() const {
  db_.clear();
  ...
  for (int lineno = 1;; lineno++) {
    ...
    if (sscanf(line.c_str(), "%s %d %d", machine, &numcpus, &powerIndex) != 3) {
    ...
    host_info_db_t::value_type object(in.s_addr, info_t(numcpus, powerIndex));
    db_.insert(object);
  }
}

bool DmucsHostsFile::getDataForHost(const struct in_addr &ipAddr, int *numCpus,
...
bool DmucsHostsFile::hasFileChanged() const {
  struct stat st;
