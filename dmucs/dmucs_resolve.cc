...
const std::string &getHostName(std::string &resolvedName,
  ...
  res8 = gethostbyaddr_r((char *)&(ipAddr.s_addr), sizeof(ipAddr.s_addr),
                         AF_INET, &he, buffer, 128, &res, &errno);
#else
#error HELP -- do not know how to compile gethostbyaddr_r
  ...
  res = gethostbyaddr((char *)&(ipAddr.s_addr), sizeof(ipAddr.s_addr), AF_INET);
  if (res != NULL) {
...
#error HELP -- do not know how to compile gethostbyaddr
  ...
  resolvedName = (res == NULL || res8 != 0) ? inet_ntoa(ipAddr) : he.h_name;
