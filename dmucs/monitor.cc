...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    DMUCS_DEBUG((stderr, "doing Sopen with %s, %s\n", serverName.str().c_str(),
                 clientPortStr.str().c_str()));
...
static void dumpSummaryInfo(std::ostringstream &availHosts,
                            std::ostringstream &overHosts,
                            std::ostringstream &unavailHosts,
                            std::ostringstream &silentHosts,
                            std::ostringstream &unkHosts) {
...
void parseResults(const char *resultStr) {
  ...
  std::ostringstream unkHosts, availHosts, unavailHosts, overHosts, silentHosts;
    ...
    switch (firstChar) {
      ...
      case 'H': {
        ...
        NSDictionary *Info = [NSDictionary
            dictionaryWithObjectsAndKeys:HostName, @"HostName", StatusNum,
                                         @"Status", nil];
        [Notifier
            postNotificationName:@"dmucsMonitorHostStatus"
                          object:@"DMUCS"
                        userInfo:Info
      ...
      case 'C': {
        ...
        while (1) {
          ...
          NSDictionary *Info = [NSDictionary
              dictionaryWithObjectsAndKeys:HostName, @"HostName", TierNum,
                                           @"Tier", CpusNum, @"Cpus", nil];
          [Notifier
              postNotificationName:@"dmucsMonitorHostTier"
                            object:@"DMUCS"
                          userInfo:Info
                           options:(NSUInteger)NSNotificationPostToAllSessions];
#endif
...
void usage(const char *prog) {
  fprintf(stderr,
          "Usage: %s [-s|--server <server>] [-p|--port <port>] "
          "[-D|--debug]\n\n",
          prog);
}
